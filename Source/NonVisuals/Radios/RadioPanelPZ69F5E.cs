                    ...
                    do
                    {
                        ...
                        if (Interlocked.Read(ref _tacanDial1WaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockTacanDialsObject1)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = TACAN_FREQ_1DIAL_COMMAND + (_tacanCockpitFreq1DialPos < desiredPositionDial1 ? Increase : dec);
                                    DCSBIOS.Send(str);
                                    dial1SendCount++;
                        ...
                        if (Interlocked.Read(ref _tacanDial2WaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockTacanDialsObject2)
                            {
                                if (_tacanCockpitFreq2DialPos != desiredPositionDial2)
                                {
                                    ...
                                    var str = TACAN_FREQ_2DIAL_COMMAND + (_tacanCockpitFreq2DialPos < desiredPositionDial2 ? Increase : dec);
                                    DCSBIOS.Send(str);
                                    dial2SendCount++;
                        ...
                        if (Interlocked.Read(ref _tacanDial3WaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockTacanDialsObject3)
                            {
                                if (_tacanCockpitFreq3DialPos != desiredPositionDial3)
                                {
                                    ...
                                    var str = TACAN_FREQ_3DIAL_COMMAND + (_tacanCockpitFreq3DialPos < desiredPositionDial3 ? Increase : dec);
                                    DCSBIOS.Send(str);
                                    dial3SendCount++;
        ...
        public void PZ69KnobChanged(bool isFirstReport, IEnumerable<object> hashSet)
        {
            ...
            lock (LockLCDUpdateObject)
            {
                ...
                foreach (var radioPanelKnobObject in hashSet)
                {
                    ...
                    {
                        PluginManager.DoEvent(DCSFPProfile.SelectedProfile.Description, HIDInstance, PluginGamingPanelEnum.PZ69RadioPanel_PreProg_F5E, (int)radioPanelKnob.RadioPanelPZ69Knob, radioPanelKnob.IsOn, null);
                    }
                }
