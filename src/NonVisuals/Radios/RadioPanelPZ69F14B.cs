    ...
    public class RadioPanelPZ69F14B : RadioPanelPZ69Base, IDCSBIOSStringListener
    {
        ...
        */
        ...
        private void SendUHFToDCSBIOS()
        {
            ...
            _uhfSyncThread = new Thread(() => UHFSyncThreadMethod(desiredDial1Value, desiredDial2Value, desiredDial3Value, desiredDial4Value));
            _uhfSyncThread.Start();
        }

        private volatile bool _shutdownUHFThread;
        private void UHFSyncThreadMethod(int desiredValueDial1, int desiredValueDial2, int desiredValueDial3, int desiredValueDial4)
        {
            try
            {
                try
        ...
        private void SendVUHFToDCSBIOS()
        {
            ...
            _vuhfSyncThread = new Thread(() => VUHFSyncThreadMethod(desiredDial1Value, desiredDial2Value, desiredDial3Value, desiredDial4Value));
            _vuhfSyncThread.Start();
        }

        private volatile bool _shutdownVUHFThread;
        private void VUHFSyncThreadMethod(int desiredValueDial1, int desiredValueDial2, int desiredValueDial3, int desiredValueDial4)
        {
            try
            {
                try
        ...
        private void SendLink4ToDCSBIOS()
        {
            ...
            _rioLink4SyncThread = new Thread(() => RioDatalink4SyncThreadMethod(_rioLink4HundredsFrequencyStandby, dial2, dial3));
            _rioLink4SyncThread.Start();
        }

        private volatile bool _shutdownRIOLink4Thread;
        private void RioDatalink4SyncThreadMethod(int desiredPositionDial1, int desiredPositionDial2, int desiredPositionDial3)
        {
            try
            {
                try
                    ...
                    do
                    {
                        ...
                        if (Interlocked.Read(ref _rioLinkHundredsWaitingForFeedback) == 0)
                        {
                            lock (_lockRioLink4HundredsDial)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = RIO_LINK4_HUNDREDS_DIAL_COMMAND + (_rioLink4HundredsCockpitFrequency < desiredPositionDial1 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial1SendCount++;
                        ...
                        if (Interlocked.Read(ref _rioLinkTensWaitingForFeedback) == 0)
                        {
                            lock (_lockRioLink4TensDial)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = RIO_LINK4_TENS_DIAL_COMMAND + (_rioLink4TensCockpitFrequency < desiredPositionDial2 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial2SendCount++;
                        ...
                        if (Interlocked.Read(ref _rioLinkOnesWaitingForFeedback) == 0)
                        {
                            lock (_lockRioLink4OnesDial)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = RIO_LINK4_ONES_DIAL_COMMAND + (_rioLink4OnesCockpitFrequency < desiredPositionDial3 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial3SendCount++;
        ...
        private void SendPilotTacanToDCSBIOS()
        {
            ...
            _pilotTacanSyncThread = new Thread(() => PilotTacanSyncThreadMethod(_pilotTacanTensFrequencyStandby, _pilotTacanOnesFrequencyStandby, _pilotTacanXYStandby));
            _pilotTacanSyncThread.Start();
        }

        private volatile bool _shutdownPilotTACANThread;
        private void PilotTacanSyncThreadMethod(int desiredPositionDial1, int desiredPositionDial2, int desiredPositionDial3)
        {
            try
            {
                try
                    ...
                    do
                    {
                        ...
                        if (Interlocked.Read(ref _pilotTacanTensWaitingForFeedback) == 0)
                        {
                            lock (_lockPilotTacanTensDialObject)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = PILOT_TACAN_TENS_DIAL_COMMAND + (_pilotTacanCockpitTensDialPos < desiredPositionDial1 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial1SendCount++;
                        ...
                        if (Interlocked.Read(ref _pilotTacanOnesWaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockPilotTacanOnesObject)
                            {
                                if (_pilotTacanCockpitOnesDialPos != desiredPositionDial2)
                                {
                                    ...
                                    var str = PILOT_TACAN_ONES_DIAL_COMMAND + (_pilotTacanCockpitOnesDialPos < desiredPositionDial2 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial2SendCount++;
                        ...
                        if (Interlocked.Read(ref _pilotTacanXYWaitingForFeedback) == 0)
                        {
                            lock (_lockPilotTacanXYDialObject)
                            {
                                if (_pilotTacanCockpitXYDialPos != desiredPositionDial3)
                                {
                                    ...
                                    var str = PILOT_TACAN_XY_DIAL_COMMAND + (_pilotTacanCockpitXYDialPos < desiredPositionDial3 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial3SendCount++;
        ...
        private void SendRioTacanToDCSBIOS()
        {
            ...
            _rioTacanSyncThread = new Thread(() => RioTacanSyncThreadMethod(_rioTacanTensFrequencyStandby, _rioTacanOnesFrequencyStandby, _rioTacanXYStandby));
            _rioTacanSyncThread.Start();
        }

        private volatile bool _shutdownRIOTACANThread;
        private void RioTacanSyncThreadMethod(int desiredPositionDial1, int desiredPositionDial2, int desiredPositionDial3)
        {
            try
            {
                try
                    ...
                    do
                    {
                        ...
                        if (Interlocked.Read(ref _rioTacanTensWaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockRioTacanTensDialObject)
                            {
                                ...
                                {
                                    dial1OkTime = DateTime.Now.Ticks;
                                    var str = RIO_TACAN_TENS_DIAL_COMMAND + (_rioTacanCockpitTensDialPos < desiredPositionDial1 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial1SendCount++;
                        ...
                        if (Interlocked.Read(ref _rioTacanOnesWaitingForFeedback) == 0)
                        {
                            ...
                            lock (_lockRioTacanOnesObject)
                            {
                                if (_rioTacanCockpitOnesDialPos != desiredPositionDial2)
                                {
                                    ...
                                    var str = RIO_TACAN_ONES_DIAL_COMMAND + (_rioTacanCockpitOnesDialPos < desiredPositionDial2 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial2SendCount++;
                        ...
                        if (Interlocked.Read(ref _rioTacanXYWaitingForFeedback) == 0)
                        {
                            lock (_lockRioTacanXYDialObject)
                            {
                                if (_rioTacanCockpitXYDialPos != desiredPositionDial3)
                                {
                                    ...
                                    var str = RIO_TACAN_XY_DIAL_COMMAND + (_rioTacanCockpitXYDialPos < desiredPositionDial3 ? DCSBIOSCommand.DCSBIOS_INCREMENT : DCSBIOSCommand.DCSBIOS_DECREMENT);
                                    DCSBIOS.Send(str);
                                    dial3SendCount++;
